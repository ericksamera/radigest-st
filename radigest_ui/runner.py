...
def is_done(run_dir: Path) -> bool:
    ...
    if status.get("state") != "DONE":
        return False
    return all(path.exists() and path.stat().st_size > 0 for path in expected_outputs(run_dir).values())
...
def argv_from_manifest(manifest: dict, run_dir: Path) -> list[str]:
    ...
    argv = [
        ...
        "--enzymes", _enzyme_arg(manifest, run_dir),
        "--target-genome-pct", str(params["target_genome_pct"]),
        "--coverage-tolerance-pct", str(params["coverage_tolerance_pct"]),
        "--desired-depth", str(params["desired_depth"]),
        "--samples", str(params["samples"]),
        "--read-layout", params["read_layout"],
        "--read-length", str(params["read_length"]),
        "--usable-read-fraction", str(params["usable_read_fraction"]),
        "--denominator", params["denominator"],
        "--min", str(params["min_bp"]),
        "--max", str(params["max_bp"]),
        "--score-min", str(params["score_min_bp"]),
        "--score-max", str(params["score_max_bp"]),
        "--size-model", params["size_model"],
        "--size-mean", str(params["size_mean_bp"]),
        "--size-sd", str(params["size_sd_bp"]),
        "--size-edge-sd", str(params["size_edge_sd_bp"]),
        "--objective", params["objective"],
        "--weight-coverage", str(params["weight_coverage"]),
        "--weight-depth", str(params["weight_depth"]),
        "--weight-overcoverage", str(params["weight_overcoverage"]),
        "--weight-insert", str(params["weight_insert"]),
        "--jobs", str(params["jobs"]),
        "--build-workers", str(params["build_workers"]),
    ...
    if params["budget_mode"] == "flowcell":
        ...
        argv += ["--lane-read-pairs", str(params["lane_read_pairs"]), "--lanes", str(params["lanes"])]
...
def run_or_reuse(run_key: str) -> RunResult:
    ...
    if is_done(run_dir):
        return RunResult(run_key=run_key, run_dir=run_dir, state="DONE", exit_code=0, reused=True)
    ...
    with FileLock(str(lock_path)):
        ...
        if not manifest_path.exists():
            write_status(run_dir, {"state": "ERROR", "error": "manifest.json missing", "finished_at": utc_now()})
            return RunResult(run_key=run_key, run_dir=run_dir, state="ERROR", exit_code=None)
...
def _run_radigest_design(run_key: str, run_dir: Path, manifest: dict) -> RunResult:
    ...
    if proc.returncode != 0:
        ...
        return RunResult(run_key=run_key, run_dir=run_dir, state="ERROR", exit_code=proc.returncode)

    missing = [str(path) for path in expected_outputs(run_dir).values() if not path.exists()]
    if missing:
        write_status(run_dir, {
            "state": "ERROR",
            "run_key": run_key,
    ...
    write_status(run_dir, {
        ...
        "finished_at": utc_now(),
        "exit_code": proc.returncode,
        "outputs": {name: str(path) for name, path in expected_outputs(run_dir).items()},
        "argv": argv,
    })
...
def _run_mock(run_key: str, run_dir: Path, manifest: dict) -> RunResult:
    ...
    for i, (a, b) in enumerate(pairs, start=1):
        ...
        feasible = pct_error <= float(params["coverage_tolerance_pct"]) and predicted_depth >= target_depth
        loss = pct_error / max(target_pct, 1e-9) + max(0, target_depth - predicted_depth) / max(target_depth, 1e-9)
        fit_score = 1 / (1 + loss)
        rows.append({
            "rank": i,
            "enzyme_a": a,
            ...
            "max_samples": max(1, int(read_pairs_total / max(target_depth * weighted_fragments, 1))),
    ...
    (run_dir / "stderr.txt").write_text("mock runner; no radigest-design binary executed\n", encoding="utf-8")
...
def _write_mock_summary_tsv(path: Path, rows: list[dict]) -> None:
    fields = [
        "rank", "enzyme_pair", "feasible", "decision_reason", "target_pct", "predicted_pct",
        "pct_error", "target_depth", "predicted_depth", "read_pairs_per_sample", "max_samples",
        "weighted_fragments", "mean_insert_bp", "insert_status", "fit_score",
    ]
...
def _write_mock_full_tsv(path: Path, rows: list[dict], params: dict) -> None:
    fields = [
        "rank", "enzyme_a", "enzyme_b", "feasible", "decision_reason", "fit_score", "fit_loss",
        "target_genome_pct", "predicted_weighted_genome_pct", "coverage_error_pct_points",
        "coverage_error_rel", "overcoverage_rel", "undercoverage_rel", "target_mean_locus_depth",
        "predicted_mean_locus_depth", "depth_margin", "depth_shortfall_rel", "read_pairs_per_sample",
        "required_pairs_per_sample_full_target", "weighted_bases", "weighted_fragments",
        "mean_weighted_length", "raw_bases_in_window", "raw_fragments_in_window",
        "budget_supported_genome_pct", "budget_supported_weighted_bases", "max_samples_per_lane_full_target",
        "max_samples_total_full_target", "lanes_required_full_target", "adapter_threshold_bp",
        "overlap_threshold_bp", "mean_insert_category", "insert_penalty", "records", "cached_cut_sites",
    ...
    for row in rows:
        converted.append({
            **row,
            ...
            "overcoverage_rel": max(0.0, row["predicted_pct"] - row["target_pct"]) / max(row["target_pct"], 1e-9),
            "undercoverage_rel": max(0.0, row["target_pct"] - row["predicted_pct"]) / max(row["target_pct"], 1e-9),
            ...
            "depth_shortfall_rel": max(0.0, row["target_depth"] - row["predicted_depth"]) / max(row["target_depth"], 1e-9),
            "required_pairs_per_sample_full_target": row["target_depth"] * row["weighted_fragments"],
            "mean_weighted_length": row["mean_insert_bp"],
            "budget_supported_genome_pct": min(row["predicted_pct"], row["predicted_pct"] * row["predicted_depth"] / max(row["target_depth"], 1e-9)),
            ...
            "overlap_threshold_bp": int(params["read_length"]) * 2 if params["read_layout"] == "pe" else "",
